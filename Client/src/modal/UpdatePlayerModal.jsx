...
function UpdatePlayerModal({open, currentPlayer, teamData, setOpenModal, getTestPlayers}) {
    if (!open) {
        return null;
      }
      ...
      const [selectedPosition, setSelectedPosition] = useState(currentPlayer.position);
      const [selectedClass, setSelectedClass] = useState(currentPlayer.class);
      const [selectedHandedness, setSelectedHandedness] = useState(currentPlayer.handedness);
      const [playerFirstName, setPlayerFirstName] = useState(currentPlayer.firstName);
      ...
      const [playerJerseyNumber, setPlayerJerseyNumber] = useState(currentPlayer.jerseyNumber);
      ...
      const submitUpdatedPlayer = async(e, id) => {
        ...
        try {
          await axios.put("http://localhost:9200/players/"+ currentPlayer.playerID, updatedPlayer);
  ...
  return (
    <div className='update_overlay'>
      ...
      <div className="update_player_form_controls">
        <button className="submit_player_btn" onClick={submitUpdatedPlayer}>
          Submit
        </button>
        <button className="submit_player_btn" onClick={() => setOpenModal(false)}>
          Cancel
        </button>
      </div>
    </form>
