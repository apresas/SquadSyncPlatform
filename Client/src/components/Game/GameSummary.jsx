...
function GameSummary({
  ...
  useEffect(() => {
    setScores()
  }, [homeTeam, awayTeam, currentEvents]);

  useEffect(() => {
    setGameScore({
      homeScore: lineScore.homeScore.first + lineScore.homeScore.second + lineScore.homeScore.third,
      awayScore: lineScore.awayScore.first + lineScore.awayScore.second + lineScore.awayScore.third
    })
  }, [lineScore])
  ...
  const setScores = () => {
    ...
    {currentEvents.map((event) => {
      if(event.scoreTeam === homeTeam.teamID && event.period === '1st') {
        homeFirst += 1
      } else if(event.scoreTeam === homeTeam.teamID && event.period === '2nd') {
        homeSecond += 1
      }
      else if(event.scoreTeam === homeTeam.teamID && event.period === '3rd') {
        homeThird += 1
      }

      if(event.scoreTeam === awayTeam.teamID && event.period === '1st') {
        awayFirst += 1
      } else if(event.scoreTeam === awayTeam.teamID && event.period === '2nd') {
        awaySecond += 1
      }
      else if(event.scoreTeam === awayTeam.teamID && event.period === '3rd') {
        awayThird += 1
      }
    })}
    setLineScore({
